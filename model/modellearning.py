...
# >> 총 200,222개의 리뷰(훈련: 15만개, 테스트: 5만개)로
...
#           평점이 긍정(9~10)점, 부정(1~4점), 5~8점은 제거
...
# [7156791, 액션이 없는 데도 재미 있는 몇안되는 영화, 1]
...
# konply에는 여러 클래스가 존재하지만 그중 okt(open korean text)를
# 사용하여 간단한 문장분석을 실행한다.
...
# 자주 출력하는 단어 50개를 matplotlib을 통해 그래프로 그리기
# 한글 폰트를 로드해야 깨지지 않고 출력 됨
...
# matplotlib 이 두가지를 많이 쓴다.
# seaborn

# 자주 사용되는 토큰 5000개를 사용해서 데이터를 벡터화 시킨다
# 문서 집합에서 단어 토큰을 생성하고 각 단어의 수를 세어
# BOW(Bag of Words) 인코딩한 벡터(배열)를 만드는 역할을 한다.
# 스칼라= 하나의 값  배열 = 벡터  매트릭스 = 벡터의 모임
...
print('>> 형변환 시작')
x_train = np.asarray(train_x).astype('float32')  # np 수치 연산을 쉽게 하기위한 타입
...
# 모델 층을 구성   입력층 출력층을 제외한 층의 갯수가 2개 이상인것을 deep learnning 이라고 하고 지금처럼 층이1개뿐인것을 shallow learnning이라한다.
model.add(layers.Dense(64, activation='relu', input_shape=(5000,)))  # 1층생성 (입력층 input)
model.add(layers.Dense(64, activation='relu'))  # 2층 생성
model.add(layers.Dense(1, activation='sigmoid'))  # 3층 생성 (out put 출력층)
